/* eslint-disable */
const testCase = '交易管理-交易录入：簿记交易_自定义产品_非年化';
...
describe(testCase, () => {
  ...
  it(testCase, () => {
    ...
    cy.contains('自定义产品').click();

    // 填充字段
    cy.fixture('modelXyUnannualBook.json').as('bookData');
    cy.get('@bookData').then(data => {
      ...
      // 起始日
      cy.get('[data-test=table-1] > :nth-child(13)')
        .click({ force: true })
        .focused()
        .clear()
      ...
      cy.get('[data-test=table-1] > :nth-child(21)')
        ...
        .blur();
      // cy.get('[data-test=table-1] > :nth-child(21)').should('contain', zhPosition.notionalAmount);
      // 备注
      ...
      // 起始日
      cy.get('[data-test=table-1] > :nth-child(13)').should('contain', zhPosition.effectiveDate);
      ...
      cy.get('[data-test=table-1] > :nth-child(15)').should(
        'contain',
        Cypress.moment(position.effectiveDate)
          .subtract(-position.term, 'days')
          .format('YYYY-MM-DD'),
      ...
      // 实际期权费
      cy.get('[data-test=table-1] > :nth-child(17)').should('contain', zhPosition.premium);
      // 名义本金
      cy.get('[data-test=table-1] > :nth-child(20)').should(
        'contain',
      ...
      cy.get('[data-test=table-1] > :nth-child(21)').should('contain', zhPosition.notionalAmount);
      // 备注
      cy.get('[data-test=table-1] > :nth-child(23)').should('contain', zhPosition.comment);
    });
  });
});
