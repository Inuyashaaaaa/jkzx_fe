...
const testCase = '交易管理-交易录入:簿记交易成功_跨式_非年化（OTMS-6462）';
...
describe(testCase, () => {
  ...
  it('交易录入：簿记交易', () => {
    ...
    //添加期权结构
    cy.fixture('modelTransN.json').as('modelTransN');
    cy.get('@modelTransN').then(data => {
      const { position, book } = data;
      cy.contains(position.struType).click();
      //年化/非年化
      cy.get('[data-test=table-1] > :nth-child(2)').click();
      cy.get('.ant-select-dropdown-menu > :nth-child(2)').as('isYear');
      cy.get('@isYear').click();
      ...
      //低行权价
      cy.get('[data-test=table-1] > :nth-child(12)').as('lowStrike');
      cy.get('@lowStrike')
        .click()
        .focused()
        .clear()
      ...
      //高行权价
      cy.get('[data-test=table-1] > :nth-child(13)').as('highStrike');
      cy.get('@highStrike')
        .click()
        .focused()
        .clear()
      ...
      //起始日
      cy.get('[data-test=table-1] > :nth-child(16)').as('effectiveDate');
      ...
      //到期日
      cy.get('[data-test=table-1] > :nth-child(17)').as('expirationDate');
      ...
      //结算日期
      cy.get('[data-test=table-1] > :nth-child(18)').as('settlementDate');
      ...
      //实际权期费
      cy.get('[data-test=table-1] > :nth-child(20)').as('premium');
      cy.get('@premium')
        .click()
        .focused()
      ...
      //名义本金
      cy.get('[data-test=table-1] > :nth-child(24)').as('principal');
      cy.get('@principal')
        .click()
        .focused()
      ...
      //交易数量
      cy.get('[data-test=table-1] > :nth-child(25)').as('tradCapacity');
