...
const testCase = '交易管理-交易录入:簿记交易成功_亚式_非年化（OTMS-6461）';
...
describe(testCase, () => {
  ...
  it('交易录入：簿记交易', () => {
    ...
    //添加期权结构
    cy.fixture('modelAsianN.json').as('modelAsianN');
    cy.get('@modelAsianN').then(data => {
      ...
      cy.get('.ant-select-dropdown-menu > :nth-child(2)').as('type');
      cy.get('@type').click();
      ...
      //行权价
      cy.get('[data-test=table-1] > :nth-child(12)').as('strike');
      cy.get('@strike')
        .click()
        .focused()
        .clear()
      ...
      //起始日
      cy.get('[data-test=table-1] > :nth-child(15)').as('effectiveDate');
      ...
      //到期日
      cy.get('[data-test=table-1] > :nth-child(16)').as('expirationDate');
      ...
      //结算日期
      cy.get('[data-test=table-1] > :nth-child(17)').as('settlementDate');
      ...
      //交易数量
      cy.get('[data-test=table-1] > :nth-child(24)').as('tradCapacity');
      cy.get('@tradCapacity').should('contain', position.tradCapacity);

      //观察频率
      cy.get('[data-test=table-1] > :nth-child(25)').click();
      cy.get('.ant-select-dropdown-menu > :nth-child(2)').as('watchRate');
      cy.get('@watchRate').click();

      //观察日
      cy.get('[data-test=table-1] > :nth-child(26)').as('watchDate');
