...
const testCase = '交易管理-交易录入:簿记交易成功_区间累积_年化（OTMS-4446）';
...
describe(testCase, () => {
  ...
  it('交易录入：簿记交易', () => {
    ...
    //添加期权结构
    cy.fixture('modelRangeAccruals.json').as('modelRangeAccruals');
    cy.get('@modelRangeAccruals').then(data => {
      ...
      //标的物
      cy.get('[data-test=table-1] > :nth-child(5)').as('marker');
      cy.get('@marker')
        .click()
        .focused()
      ...
      //期初价格
      cy.get('[data-test=table-1] > :nth-child(8)').as('initialPrice');
      cy.get('@initialPrice')
        .click()
        .focused()
        .clear()
      ...
      //起始日
      cy.get('[data-test=table-1] > :nth-child(12)').as('effectiveDate');
      ...
      //到期日
      cy.get('[data-test=table-1] > :nth-child(13)').as('expirationDate');
      ...
      //结算日期
      cy.get('[data-test=table-1] > :nth-child(14)').as('settlementDate');
      ...
      //实际权期费
      cy.get('[data-test=table-1] > :nth-child(16)').as('premium');
      cy.get('@premium')
        .click()
        .focused()
      ...
      //区间收益类型
      cy.get('[data-test=table-1] > :nth-child(19)').click();
      cy.get('.ant-select-dropdown-menu > :nth-child(2)').as('optionType');
      cy.get('@optionType').click();

      //区间收益
      cy.get('[data-test=table-1] > :nth-child(20)').as('EARNINGS');
      cy.get('@EARNINGS')
        .click()
        .focused()
      ...
      //名义本金
      cy.get('[data-test=table-1] > :nth-child(22)').as('principal');
      cy.get('@principal')
        .click()
        .focused()
      ...
      //交易数量
      cy.get('[data-test=table-1] > :nth-child(23)').as('tradCapacity');
      cy.get('@tradCapacity').should('contain', position.tradCapacity);
      //低障碍价
      cy.get('[data-test=table-1] > :nth-child(25)').as('lowBarrier');
      cy.get('@lowBarrier')
        .click()
        .focused()
      ...
      //高障碍价
      cy.get('[data-test=table-1] > :nth-child(26)').as('highBarrier');
      cy.get('@highBarrier')
        .click()
        .focused()
      ...
      //观察日
      cy.get('[data-test=table-1] > :nth-child(28)').as('watchDate');
