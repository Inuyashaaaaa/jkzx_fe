/* eslint-disable */
const testCase = '交易管理-交易录入：簿记交易_单鲨_非年化';
...
describe(testCase, () => {
  ...
  it(testCase, () => {
    ...
    // 填充字段
    cy.fixture('sharkUnannualBook.json').as('bookData');
    cy.get('@bookData').then(data => {
      ...
      cy.get('@起始日')
        ...
        .type('{enter}');
      // 到期日
      cy.get('[data-test=table-1] > :nth-child(16)').as('到期日');
      cy.get('@到期日')
        .click()
        .focused()
        .clear()
        ...
        .type('{enter}');
      // 结算日
      cy.get('[data-test=table-1] > :nth-child(17)').as('结算日');
      cy.get('@结算日')
        .click()
        .focused()
        .clear()
      ...
      cy.get('[data-test=table-1] > :nth-child(16)').should('contain', zhPosition.expirationDate);
      cy.get('[data-test=table-1] > :nth-child(17)').should('contain', zhPosition.settlementDate);
