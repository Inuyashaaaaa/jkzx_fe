...
const testCase = '交易管理-交易录入:簿记交易成功_四层阶梯_年化（OTMS-6511）';
...
describe(testCase, () => {
  ...
  it('交易录入：簿记交易', () => {
    ...
    //添加期权结构
    cy.fixture('modelFourSteps.json').as('modelFourSteps');
    cy.get('@modelFourSteps').then(data => {
      ...
      cy.get('.ant-select-dropdown-menu > :nth-child(1)').as('type');
      cy.get('@type').click();
      ...
      //行权价1
      cy.get('[data-test=table-1] > :nth-child(12)').as('strike1');
      cy.get('@strike1')
        .click()
        .focused()
        .type(position.strike1)
        .wait(500)
        .type('{enter}');

      //行权价2
      cy.get('[data-test=table-1] > :nth-child(13)').as('strike2');
      cy.get('@strike2')
        .click()
        .focused()
        .type(position.strike2)
        .wait(500)
        .type('{enter}');

      //行权价3
      cy.get('[data-test=table-1] > :nth-child(14)').as('strike3');
      cy.get('@strike3')
        .click()
        .focused()
        .type(position.strike3)
        .wait(500)
        .type('{enter}');

      //起始日
      cy.get('[data-test=table-1] > :nth-child(17)').as('effectiveDate');
      ...
      //到期日
      cy.get('[data-test=table-1] > :nth-child(18)').as('expirationDate');
      ...
      //结算日期
      cy.get('[data-test=table-1] > :nth-child(19)').as('settlementDate');
      ...
      //实际权期费
      cy.get('[data-test=table-1] > :nth-child(21)').as('premium');
      cy.get('@premium')
        .click()
        .focused()
      ...
      //行权收益1
      cy.get('[data-test=table-1] > :nth-child(25)').as('exercisePrifit1');
      cy.get('@exercisePrifit1')
        .click()
        .focused()
        .type(position.exercisePrifit1)
        .wait(500)
        .type('{enter}');

      //行权收益2
      cy.get('[data-test=table-1] > :nth-child(26)').as('exercisePrifit2');
      cy.get('@exercisePrifit2')
        .click()
        .focused()
        .type(position.exercisePrifit2)
        .wait(500)
        .type('{enter}');

      //行权收益3
      cy.get('[data-test=table-1] > :nth-child(27)').as('exercisePrifit3');
      cy.get('@exercisePrifit3')
        .click()
        .focused()
        .type(position.exercisePrifit3)
        .wait(500)
        .type('{enter}');

      //名义本金
      cy.get('[data-test=table-1] > :nth-child(29)').as('principal');
      cy.get('@principal')
        .click()
        .focused()
      ...
      //交易数量
      cy.get('[data-test=table-1] > :nth-child(30)').as('tradCapacity');
